...
class Connection(object):
    def __init__(self, upstream_node, downsteam_node):
        self.upstream_node = upstream_node
        self.downstream_node = downsteam_node
    ...
    def __str__(self):
        return '(%u-%u) -> (%u-%u) = %f' % (self.upstream_node.layer_index,self.upstream_node.node_index,
...
class Normalizer(object):
    ...
    def denorm(self, vec):
        binary = map(lambda i: 1 if i > 0.5else 0, vec)
...
def train_data_set():
    ...
    for i in range(0, 256, 8):
        n = normalizer.norm(int(random.uniform(0,256)))#random.uniform(x,y)随机生成x-y区间的浮点数
        data_set.append(n)
        labels.append(n)
